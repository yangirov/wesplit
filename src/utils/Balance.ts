import { Event, ParticipantsBalance, ParticipantDebt } from '../models/Event';

export function getEventBalance(currentEvent: Event): ParticipantsBalance {
  const purchases = Object.keys(
    (currentEvent && currentEvent.purchases) || []
  ).map((purchaseId) =>
    Object.assign({ id: purchaseId }, currentEvent.purchases[purchaseId])
  );

  const participantsBalance: ParticipantsBalance = {};

  //	вычисление баланса мероприятия для каждого учасника
  purchases.forEach((purchase) => {
    purchase.participants.forEach((participant) => {
      let debt = participant === purchase.payer && purchase.amount;
      participantsBalance[participant] =
        (participantsBalance[participant] || 0) +
        (Number(debt) - purchase.amount / purchase.participants.length);
    });
...
export function getEventsParticipantsDebts(
  participantsBalance: ParticipantsBalance,
  currentEvent: Event
): ParticipantDebt[] {
  const reducedBalance = Object.assign({}, participantsBalance);

  const eventsParticipantsDebts = Array<ParticipantDebt>();

  // вычисляем долг и запиываем в массив
  const payerReducer = function payerReducer(payerParticipant: string) {
